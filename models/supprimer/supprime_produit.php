<?php  
function supprimer_produit($bd, $id_produit){
$id_categorie = $_GET['id_categorie'];
 $id_boutique = $_SESSION["id_boutique"];
    $stmt = $bd->prepare("DELETE FROM produit WHERE id = ? AND id_boutique LIKE ?");
    $stmt->execute(array($id_produit, $id_boutique ));

    if ($stmt->rowCount() === 0) {
        throw new Exception("Échec de la mise à jour des comptes.");
    }else{
        $stmt->closeCursor();
        header ('Location: /Kephale/produit&id_categorie='.$id_categorie  );
    }
}
